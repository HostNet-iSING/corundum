...
class Interface:
    def __init__(self, driver, index, hw_regs):
        ...
        fmt_log = '%(asctime)s %(filename)s %(levelname)s %(message)s'
        date_fmt = '%Y%m%d %H:%M:%S'
        self.formatter = logging.Formatter(fmt=fmt_log, datefmt=date_fmt)
    ...
    async def init(self):
        ...
        self.max_tx_mtu = 32768;#await self.if_ctrl_rb.read_dword(MQNIC_RB_IF_CTRL_REG_MAX_TX_MTU)
        self.max_rx_mtu = 32768;#await self.if_ctrl_rb.read_dword(MQNIC_RB_IF_CTRL_REG_MAX_RX_MTU)
        ...
        await self.set_mtu(min(self.max_tx_mtu, self.max_rx_mtu, 32768))
    ...
    #     self.log.info("0426:index:%d, ring.stride: %d, seg: %d, offset: %d, length: %d, ptr: %d",index, ring.stride, seg, offset, length, ptr)
    ...
    #     #    struct.pack_into("<LLQLQLQ", ring.buf, index*ring.stride+k*64, opcode, length, ptr+offset, dst_qpn, src_port_rmac, rip, raddr if length else 0)
    #     #    offset += seg

    #     ring.prod_ptr += 1

    #     await ring.write_prod_ptr()
    #     await ring.read_cons_ptr()
    ...
    async def start_xmit(self, skb, tx_ring=None, csum_start=None, csum_offset=None):
        ...
        # opcode = 0x00  
        # length = len(data)
        # laddr = pkt.get_absolute_address(0)
        # dst_qpn = 0x01  
        src_port = 0x1234  
        rmac = 0x1122334455667788 
        # rip = 0x0A0A0A0A
        # raddr = 0x123456789ABCDEF0
        udp_dst_port = 0x5678  
...
# 将24位的dst_qpn拆分成两个16位字段
        dst_qpn_high = (dst_qpn >> 8) & 0xFFFF  # 高16位
        dst_qpn_low = dst_qpn & 0xFF  # 低8位

# 将48位的rmac拆分成一个32位字段和一个16位字段
        rmac_high = (rmac >> 16) & 0xFFFFFFFF  # 高32位
        rmac_low = rmac & 0x0000FFFF  # 低16位

# 使用struct.pack打包数据
        packed_data = struct.pack(
    "<B I Q H B H Q H I Q H",
    opcode,
    length,
    laddr,
    dst_qpn_high,  # dst_qpn的高16位
    dst_qpn_low,   # dst_qpn的低8位
    src_port,
    ...
    udp_dst_port,
        ...
        index = ring.prod_ptr & (len(ring.buf) // ring.stride - 1)
        ptr = pkt.get_absolute_address(0) + 10
        offset = 0
        seg = min(length-offset, 64) if ring.desc_block_size > 1 else length-offset
    ...
    csum_cmd, length, ptr+offset, dst_qpn, rmac, rip, raddr if length else 0
        )
        offset += seg
