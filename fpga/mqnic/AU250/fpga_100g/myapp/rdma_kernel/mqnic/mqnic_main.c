...
//crete mqnic rdma dev  
mqnic->mqnic_rdma = ib_alloc_device(mqnic_rdma, ibdev);
...
INIT_LIST_HEAD(&mqnic->mqnic_rdma->pending_mmaps);
...
static int mqnic_pci_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	struct page **pagelist = kmalloc(PAGE_NUM * sizeof(struct page *), GFP_KERNEL);
	for (int i = 0; i < PAGE_NUM; i++)
	{
		pagelist[i] = alloc_page(GFP_DMA);
		if (pagelist[i] == NULL)
		{
			printk(KERN_ERR "failed to alloc page %d\n", i);
			goto fail_common;
		}
	}
	ret = sg_alloc_table_from_pages(&table, pagelist, PAGE_NUM, 0, PAGE_NUM * PAGE_SIZE, GFP_KERNEL);
	if (ret < 0)
	{
		printk(KERN_ERR "failed to alloc sgtable\n");
		goto fail_common;
	}
	ret = dma_map_sgtable(dev, &table, DMA_TO_DEVICE, 0);
	if (ret < 0)
	{
		printk(KERN_ERR "failed to map sgtable\n");
		goto fail_common;
	}
	printk(KERN_INFO "mapped entries: %d origin entries: %d\n", table.nents, table.orig_nents);
