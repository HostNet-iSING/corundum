...
async def run_test_nic(dut):
    ...
    for k in range(16):
        ...
        data = bytearray([(k+1) % 256 for x in range(16384)])
        tb.log.info("Packet: %s", data)
        
        await tb.driver.interfaces[0].start_xmit(data, k)

        #data = bytearray([x % 256 for x in range(4096)])
        #tb.log.info("Packet: %s", data)
        
        #await interface.start_xmit(data, 2)

        #data = bytearray([x % 256 for x in range(4096)])
        #tb.log.info("Packet: %s", data)
        
        #await interface.start_xmit(data, 3)

        pkt = await tb.port_mac[interface.index*interface.port_count].tx.recv()
        tb.log.info("tx send Packet0428: %s", pkt)
...
def test_mqnic_core_pcie_us(request, if_count, ports_per_if, axis_pcie_data_width,
    ...
    parameters['MAX_TX_SIZE'] = 16384
    parameters['MAX_RX_SIZE'] = 16384
